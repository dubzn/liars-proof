...
pub impl ConditionTraitImpl of ConditionTrait {
    fn create(ref store: Store) -> Condition {
        ...
        let quantity = random.get_random_number(quantity_list(condition).len());
        let comparator = random.get_random_number(comparator_list(condition).len());
        let value = random.get_random_number(value_list(condition).len());
        let suit = random.get_random_number(suit_list(condition).len());
